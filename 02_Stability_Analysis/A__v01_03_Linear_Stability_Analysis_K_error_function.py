...
24Feb22:        6) Allowed mode number to be manually edited via CSV file.
                ...
                Code specific for only K_error_function.
    ...
    if not first_mode_check and not second_mode_check and not third_mode_check and not other_mode_check:
        ...
        if eigenval_all.shape[0] == 4:
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                ...
                mode_num = "2"
        elif eigenval_all.shape[0] == 3 and mu_bar <=44000:
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "Other"
            elif (idx >= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "1"
...
def append_data(stability_params,mu_bar,save_command,eigenvalues, eigenvalues_idx,eigenvectors,all_data):
    ...
    for true_idx,eig_idx in enumerate(eigenvalues_idx):
        ...
        if save_command == "Y":
            # mode_num = 0
            mode_num = check_mode_number(mu_bar,eigenvalues_idx,eigenvalues.real,eig_idx) #Old Method of classifying mode number
        else:
            mode_num = 0
...
def plot_gen_eigenspectrum_map(all_eigen_data,rigidity_type,output_dir):
    ...
    ax.set_xlim(0,80001)
    ax.set_ylim(-6,10)
    ax.set_xticks(np.linspace(0,80000,9))
...
# # mu_bar_of_int = 18000
# for mu_bar_of_int in np.arange(15000,50001,500):
