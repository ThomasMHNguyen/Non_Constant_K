...
INPUT
ARGUMENT(S):    1) Rigidity Type: The specific rigidity profile that will be 
...
24Feb22:        6) Allowed mode number to be manually edited via CSV file.
                ...
                Code specific for only K_dirac_left_l_stiff.
...
def check_mode_number(mu_bar,all_eigs_idx,eigenval_all,idx):
    """
    ...
    if not first_mode_check and not second_mode_check and not third_mode_check and not other_mode_check:
        ...
        if eigenval_all.shape[0] == 3:
            ...
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                ...
                mode_num = "1"
            elif (np.where(all_eigs_idx == idx)[0][0] == 1):
                mode_num = "3"
            elif (np.where(all_eigs_idx == idx)[0][0] == 2):
                mode_num = "2"
    elif first_mode_check and second_mode_check and third_mode_check and other_mode_check:
        if eigenval_all.shape[0] == 3 and mu_bar <= 32500:
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "Other"
            elif (idx >= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "1"
                ...
                mode_num = "3"
        elif eigenval_all.shape[0] == 4 and mu_bar < 31000:
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "Other"
            elif (idx >= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "1"
                ...
                mode_num = "2"
        elif eigenval_all.shape[0] == 4 and mu_bar >= 31000 and mu_bar < 37000:
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "Other"
            elif (idx >= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "3"
                ...
                mode_num = "Other"
        elif eigenval_all.shape[0] == 3 and mu_bar > 32500 and mu_bar < 37000:
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "Other"
            elif (idx >= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "3"
            ...
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                ...
                mode_num = "Other"
        elif eigenval_all.shape[0] == 4 and mu_bar >= 37000 and mu_bar <= 47500:
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "Other"
            elif (idx >= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "3"
                ...
                mode_num = "Other"
        elif eigenval_all.shape[0] == 4 and mu_bar == 48000:
            if (idx <= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "Other"
            elif (idx >= all_eigs_idx[all_eigs_idx != idx]).all():
                mode_num = "Other"
...
def append_data(stability_params,mu_bar,save_command,eigenvalues, eigenvalues_idx,eigenvectors,all_data):
    ...
    for true_idx,eig_idx in enumerate(eigenvalues_idx):
        ...
        if save_command == "Y":
            mode_num = check_mode_number(stability_params,mu_bar,eigenvalues_idx,eigenvalues.real,eig_idx) #Old Method of classifying mode number
        else:
            mode_num = 0
...
def plot_mode_num_eigenspectrum_map(all_eigen_data,rigidity_type,output_dir):
    ...
    ax = plt.gca()
    ax.grid(b=True, which='major',color='black', linewidth=0.3)
    ax.set_title(r"Filament Stability Growth Rate Analysis: {0}".format(0),fontsize = 30,pad = 30)
    ax.set_xlabel(r"$\bar{\mu}$",fontsize = 25,labelpad = 15)
    ax.set_ylabel(r"Real component of $\sigma$",fontsize = 25,labelpad = 15)
    ax.set_xlim(0,50001)
    ax.set_ylim(-6,10)
...
# # mu_bar_of_int = 18000
# for mu_bar_of_int in np.arange(15000,50500,500):
