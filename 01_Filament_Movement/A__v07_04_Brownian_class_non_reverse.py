...
FILES(S):       1) .NPY file that contains all positions of each discretized
                ...
                8) .CSV file that contains information as to when the direction 
                of the fluid velocity changed.
...
def calculate_N_ex(sim_c,params):
    ...
    Txss,Tsxs,Kxs,Kxssss,Ksxs,Ksxsss,Kssxss,brownian = params
...
class Constants:
    ...
    dt_scale_count = 0
    rot_time_counter = 0 #Count number of iterations after rotating
    rot_count = 0 #Number of times filament has rotated; even = forward shear, odd = reverse shear
    forward_rotation = True #True for forward shear & False for reverse
    
    def __init__(self,end_dir,rigid_func,slender_f,mubar,zeta_f,top, bottom,N,lp,dt,tot_run_time,dt_save_iteration):
        ...
        self.start_y = self.s*np.sin(self.theta) + np.ones(self.s.shape[0],dtype = float)*(1e-1)**6
        ...
        self.all_brownian_f_states = np.zeros((self.N,3,self.adj_iterations),dtype = float)
        ...
        self.all_data = [self.allstate,self.all_u0_states,self.all_du0ds_states,self.all_dxds_states,self.all_d2xds2_states, #0-4
                    ...
                    self.all_Kxssss_states, self.all_Ksxs_states, self.all_Ksxsss_states, self.all_Kssxss_states,self.all_brownian_f_states, #10-14
    ...
    def add_rotation_time(self):
        self.rot_time_counter += 1
       
    def reset_rotation_time(self):
        self.rot_time_counter = 0
    ...
    def flip_rotation(self):
        if self.forward_rotation:
            self.forward_rotation = False
        elif not self.forward_rotation:
            self.forward_rotation = True
...
def eval_time_semi(sim_c,prev_params,curr_params,t):
    ...
    # f_brownian = np.zeros((N,3),dtype = float)
    ...
    sim_c,fil_angle,fil_deflect = det_rotation(sim_c,future_xbar,t+1)
    
    var_to_pack = []
    var_to_pack = [future_xbar,u0,du0ds,dxds, d2xds2, #0-4
                   d3xds3, d4xds4,Txss,Tsxs,Kxs, #5-9
                   Kxssss,Ksxs,Ksxsss,Kssxss,f_brownian, #10-14
                   force,stress, #15-16
                   tension,dTds, #17-18
    ...
    curr_params = [v for i,v in enumerate(var_to_pack[:15]) if i != 6]
...
def eval_time_euler(sim_c,t):
    ...
    elastic_energy = calc_E_elastic(sim_c,d2xds2)
    sim_c,fil_angle,fil_deflect = det_rotation(sim_c,sim_c.initial_loc,t)
    #Record variables into arrays
    var_to_pack = []
    ...
    ### Calculate Brownian forces ###
    # f_brownian_1 = np.zeros((N,3),dtype = float)
    ...
    sim_c,fil_angle_1,fil_deflect_1 = det_rotation(sim_c,future_xbar,t+1)
    #Save data for next iteration
    fut_var_to_pack = []
    fut_var_to_pack = [future_xbar,u0_1,du0ds_1,dxds_1, d2xds2_1, #0-4
                   d3xds3_1, d4xds4_1,Txss_1,Tsxs_1,Kxs_1, #5-9
                   Kxssss_1,Ksxs_1,Ksxsss_1,Kssxss_1,f_brownian_1, #10-14
...
def run_simulation(target_dir,rigid_type,mu_bar,rep):
    ...
    sim_c = Constants(end_dir = target_dir,rigid_func = rigid_type,
                                slender_f = 0.01,mubar = mu_bar,zeta_f =1e5,
                                top = 199, bottom = 200,
                                N = 101,lp = 1000,dt = 1e-6,tot_run_time = 5e-2,
                                dt_save_iteration = 1e-5)
    
    dir_name = '{}/Mu_bar_{}/rep_{}'.format(sim_c.rigidity_suffix,int(sim_c.mu_bar),rep)
    ...
    prev_info = [sim_c.all_data[i][:,:,0] for i in range(0,15) if i != 6]#  unpack parameters at t = 0 #
    curr_info = [small_step_vars[i] for i in range(0,15) if i != 6] # unpack parameters at next iteration
    ...
    logging.info("Finished with the numerical simulations calculations portion \
                 for Mu_bar = {}, Replicate # {}".format(mu_bar,rep))
    ...
    parameter_df = pd.DataFrame(index = ['Random Seed','Filament Length','Filament Persistence Length','Filament s start','Filament s end',
    ...
    parameter_df.loc['Filament Persistence Length','Value'] = sim_c.lp
    ...
    parameter_df.loc['Total Run Time'] = (sim_c.iterations)*sim_c.dt
    ...
    logging.info("Finished computations and saving data for Mu_bar = {}, Replicate # {}".format(mu_bar,rep))
...
def Main():
    ...
    start_time = time.perf_counter()
    sim_c = run_simulation(args.output_directory,args.rigidity_type,args.mu_bar,args.replicate_value)
    end_time = time.perf_counter()
...
plt.plot(np.linspace(0,sim_data.tot_time,sim_data.adj_iterations),sim_data.all_data[20])
...
plt.figure(figsize = (8,8))
plt.plot(sim_data.all_data[17][:,0],'b',label = 'Initial Tension')
plt.plot(sim_data.all_data[17][:,-1],'r',label = 'Final Tension')
...
plt.plot(sim_data.all_data[0][:,0,-1] - sim_data.all_data[0][:,0,-1].mean(),sim_data.all_data[0][:,1,-1],'r',label = 'Final Position')
plt.axis('square')
